'''
Description: Prototype System of PVMPATCH
LastEditTime: 2021-10-21 16:44:03
'''
...
def PVMPATCH(cfg, save_dir, output_dir, temp_path, sum_path):
    '''
    description: 
...
# ======================= MD5 verify ======================= 
    ...
    if md5 != '':
        if md5_verify(file_path, md5, log_path) == 0 :
            exit(1)
...
# ======================= Fix ======================= 
    ...
    fixed_code_path, patch_number, fix_time = fixall(df,file_path, output_path, 1, temp_path, name, log_path)
                                         
# ======================= Validate: Analysis after Fix ======================= 

    print_to_log(name + ' patched code analysis start.', log_path)
    fixed_analysis_time, fixed_analysis_json_path, fixed_analysis_txt_path = analysis(fixed_code_path)
    if(os.path.exists(fixed_analysis_json_path) == False):
        ...
        exit(1)
    df = single_version_extract(fixed_analysis_json_path, fixed_analysis_txt_path, output_path, 'after_fix')

    print_to_log(name + ' patched code analysis costed %.2f milliseconds.' % fixed_analysis_time, log_path)
    print_to_log('patched code analysis report path:  ' + fixed_analysis_json_path, log_path)
...
def multiprocess():    
    ...
    for cfg in cfg_list:
        res = p.apply_async(PVMPATCH, args = (cfg, save_dir, output_dir, temp_path, sum_path))
        res_list.append(res) 
